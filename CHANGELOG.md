...
### Changed
- **Performance Optimization**: Mengimplementasikan konversi otomatis ke WebP terkompresi dan responsive resizing pada seluruh gambar carousel, logo, dan thumbnail berita via helper `getOptimizedImageUrl()` (dengan dukungan penuh URL absolut & prefix `/v1/`), meniadakan pra-muat idle slide carousel tambahan, menambahkan deklarasi lengkap `@font-face` dengan `font-display: swap` untuk Font Awesome, serta memperpanjang *Cache-Control* aset statis menjadi 30 hari di `.htaccess` untuk memangkas LCP dan mempercepat render halaman.
