...
- **Admin UI Logic**:
    ...
    - **Sorting Logic**: Profiles are sorted by `type` (ASC), then `order` (ASC), with a fallback to `created_at` (ASC) to ensure oldest profiles appear first when order is tied.
