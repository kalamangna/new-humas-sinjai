...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 sm:px-8 py-4 sm:py-6 bg-slate-50 border-b border-slate-200 flex items-center">
            ...
            <div>
                <h2 class="text-base sm:text-lg font-black text-slate-900 tracking-tight">Tambah Slide</h2>
                <p class="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Lengkapi rincian slide baru</p>
            </div>
        </div>

        <div class="p-4 sm:p-8 md:p-12">
            <form action="<?= base_url('admin/carousel') ?>" method="post" enctype="multipart/form-data" class="space-y-6 sm:space-y-8">
                <?= csrf_field() ?>
                ...
                <div class="space-y-4">
                    ...
                    <label class="block">
                        <div class="flex items-center px-4 py-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl sm:rounded-2xl hover:border-blue-800 hover:bg-slate-100 transition-all cursor-pointer">
                            <i class="fa-solid fa-fw fa-image text-slate-400 mr-3 sm:mr-4 flex-shrink-0"></i>
                            <span class="text-xs sm:text-sm font-bold text-slate-500 truncate" id="file-name">Klik untuk memilih file...</span>
                    ...
                    <div id="image-preview-container" class="mt-4 sm:mt-6 hidden ring-4 ring-slate-50 rounded-2xl overflow-hidden shadow-2xl border border-slate-200">
