...
<?= $this->section('page_title') ?>Tambah Live Stream<?= $this->endSection() ?>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-8 py-6 bg-slate-50 border-b border-slate-200 flex items-center">
            <div class="w-10 h-10 bg-blue-800 text-white rounded-xl flex items-center justify-center mr-4">
                <i class="fa-solid fa-fw fa-tv text-sm"></i>
            </div>
            <div>
                <h2 class="text-lg font-black text-slate-900 tracking-tight">Live Stream Baru</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Konfigurasi tayangan Facebook Live</p>
            </div>
        </div>
        ...
        <div class="p-8 md:p-12">
            ...
            <form action="<?= base_url('admin/live-streams') ?>" method="post" class="space-y-8">
                <?= csrf_field() ?>
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="title" required class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl text-sm font-bold text-slate-900 focus:border-blue-800 outline-none transition-all" value="<?= old('title') ?>" placeholder="Contoh: Siaran Langsung Pelantikan Pejabat">
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <input type="url" name="live_url" required class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl text-sm font-bold text-slate-900 focus:border-blue-800 outline-none transition-all" value="<?= old('live_url') ?>" placeholder="https://www.facebook.com/watch/live/?v=...">
                    <p class="text-[10px] text-slate-400 font-medium">Pastikan URL berasal dari domain facebook.com</p>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" <?= old('is_active') ? 'checked' : '' ?> class="w-4 h-4 text-blue-800 border-slate-300 rounded focus:ring-blue-800">
                ...
                <div class="pt-8 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-4">
                    ...
                    <button type="submit" class="px-10 py-4 bg-blue-800 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl hover:bg-blue-900 transition-all shadow-xl shadow-blue-900/20">
                        <i class="fa-solid fa-fw fa-floppy-disk mr-2 text-sm"></i>Simpan Live Stream
                    </button>
                </div>
            </form>
        </div>
