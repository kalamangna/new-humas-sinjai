...
<div class="max-w-5xl mx-auto">
    <div class="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-5 sm:px-8 py-4 sm:py-6 bg-slate-50 border-b border-slate-200 flex items-center">
            ...
            <div>
                <h2 class="text-base sm:text-lg font-black text-slate-900 tracking-tight">Ubah Berita</h2>
                <p class="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">ID: #<?= $post['id'] ?> • Terakhir diperbarui <?= date('d/m/y H:i', strtotime($post['updated_at'])) ?></p>
            </div>
        </div>

        <div class="p-4 sm:p-8 md:p-12">
            <form action="<?= base_url('admin/posts/' . $post['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-8 sm:space-y-10">
                            ...
                            <div class="flex items-center space-x-2">
                                <label class="flex-1 cursor-pointer min-w-0">
                                    <div class="flex items-center px-4 py-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl hover:border-blue-800 hover:bg-slate-100 transition-all">
                                        ...
                                        <span class="text-xs sm:text-sm font-bold text-slate-500 truncate" id="file-name"><?= $post['thumbnail'] ? 'Ganti Gambar' : 'Pilih Gambar' ?></span>
                                ...
                                <button type="button" id="paste-thumbnail-btn" class="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-blue-800 hover:text-white transition-all shadow-sm flex-shrink-0">
                    ...
                    <div class="space-y-6 sm:space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex gap-2">
                                <input type="text" id="manual-tag-input" class="flex-1 min-w-0 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-800" placeholder="Tambah tag manual...">
                ...
                <div class="pt-6 sm:pt-10 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
                    <a href="<?= base_url('admin/posts') ?>" class="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 bg-slate-100 text-slate-600 font-black text-[11px] uppercase tracking-[0.2em] rounded-xl sm:rounded-2xl hover:bg-slate-200 transition-all text-center">
                        Batal
                    </a>
                    <?php if ($post['status'] === 'published') : ?>
                        <button type="submit" class="w-full sm:w-auto px-8 sm:px-10 py-3.5 sm:py-4 bg-blue-800 text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-xl sm:rounded-2xl hover:bg-blue-900 transition-all shadow-xl shadow-blue-900/20 text-center">
                            <i class="fa-solid fa-fw fa-floppy-disk mr-2 text-sm"></i>Perbarui
                        </button>
                    <?php else : ?>
...
<script>
    ...
    document.querySelectorAll('.remove-tag').forEach(btn => btn.onclick = () => {
        btn.closest('.tag-badge').remove();
        updateTagsInput();
    });
