                    ...
                    <?php foreach ($posts as $post) : ?>
                        <tr class="hover:bg-slate-50 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right space-x-1 whitespace-nowrap w-1">
                                ...
                                <?php endif; ?>
                                <a href="<?= base_url('admin/posts/' . $post['id'] . '/edit') ?>" class="inline-flex items-center p-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-blue-800 hover:text-white transition-all shadow-sm">
...
<?php if (isset($pager) && $pager->getPageCount('posts') > 1) : ?>
    <div class="mt-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            Total: <span class="text-slate-900"><?= number_format($pager->getTotal('posts')) ?></span>
        </div>
        <div>
