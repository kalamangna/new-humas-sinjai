        ...
        <div class="p-8 md:p-12">
            <form action="<?= base_url('admin/profiles/' . $profile['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-10 needs-validation" novalidate>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <input type="text" name="name" required class="w-full px-4 py-3 bg-slate-50 border <?= session('errors.name') ? 'border-red-500 ring-2 ring-red-500/10' : 'border-slate-200' ?> rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-blue-800 outline-none transition-all" value="<?= old('name', $profile['name']) ?>">
                        ...
                        <div class="space-y-4">
                            ...
                            <input type="text" name="position" required class="w-full px-4 py-3 bg-slate-50 border <?= session('errors.position') ? 'border-red-500 ring-2 ring-red-500/10' : 'border-slate-200' ?> rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-blue-800 outline-none transition-all" value="<?= old('position', $profile['position']) ?>">
                ...
                <div id="image-container" class="space-y-4 pt-10 border-t border-slate-100">
                    ...
                    <label class="block cursor-pointer">
                        <div class="flex items-center px-4 py-3 bg-slate-50 border-2 border-dashed <?= session('errors.image') ? 'border-red-500 bg-red-50' : 'border-slate-200 hover:border-blue-800 hover:bg-slate-100' ?> rounded-xl transition-all">
                            ...
                            <span class="text-sm font-bold <?= session('errors.image') ? 'text-red-500' : 'text-slate-500' ?> truncate" id="file-name"><?= $profile['image'] ? 'Ganti Foto' : 'Pilih Foto' ?></span>
                ...
                <div id="bio-container" class="space-y-4">
                    ...
                    <textarea name="bio" id="bio" rows="10" class="w-full <?= session('errors.bio') ? 'border-red-500 ring-2 ring-red-500/10' : '' ?>"><?= old('bio', $profile['bio']) ?></textarea>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchWilayah(kecamatan, tipe, selectElement, selectedValue = null) {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    const selectedValueInAttr = selectElement.dataset.selected;
                    data.forEach(item => {
                        ...
                        if (!item.kecamatan_nama || itemKecamatan === targetKecamatan) {
                            ...
                            if (namaWilayah) {
                                ...
                                if (option.value === (selectedValue || selectedValueInAttr)) {
                                    option.selected = true;
                                }
